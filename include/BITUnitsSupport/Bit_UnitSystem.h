...
public:
	...
	// 初始化设置单位制度
	void Set_Initial( const int & Length , const int & Mass , const int & Time );
	...
	// 16.得到基础长度单位下的3D坐标值
	inline Bit_Point3D Get_3DPointValueOnBaseU( const Bit_Point3D &InputCoord)const;
	// 17.由基础长度单位下的2D坐标值得到当前坐标单位下的坐标值
	inline Bit_Point2D Get_2DPointValueOnBaseN( const Bit_Point3D &InputCoord)const;
	// 18.得到速度衰减系数
