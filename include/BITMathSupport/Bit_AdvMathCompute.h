...
#include "Bit_MathSupport_Global.h"
#include "BITPointSupport/Bit_Point3D.h"

class BITMATHSUPPORT_API Bit_AdvMathCompute
{
public:
	Bit_AdvMathCompute(void);
	~Bit_AdvMathCompute(void);
	...
	// 弧度大于2Pi的转换至2Pi空间内
	inline double TransationInto2Pi( const double& Angle_R ) const;
	...
	//去除重复坐标点返回新的坐标点数组
	void NewArrayPointDistinct( const int&Old_n , const Bit_Point3D Old_P[], int *New_n , Bit_Point3D New_P[] ) const;


	// 计算非独立事件的“与”（1-(1-P1)(1-P2)...）函数
	double TheAndComputation(const int& n, const double single_p[] )const ;
	// 计算非独立事件的“或”函数
	double TheOrComputation(const int& n, const double single_p[] )const ;
