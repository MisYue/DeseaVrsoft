...
//形状描述
class WARHEADMODELDATA_EXPORT ShapeDescribe
{
public:
	ShapeDescribe(const Bit_Point2D& datum_point = Bit_Point2D()) 
	...
	//形状描述工厂
	static ShapeDescribe* CreateShape(ShapeType type, const QString& describe);
	//向量归一
...
{
public:
	ConeDescribe(double left_r = 0, double right_r = 0, double length = 0) 
...
{
public:
	CylinderDescribe(double r = 0, double length = 0) : r_(r), length_(length) 
	{
...
{
public:
	OvalDescribe(double left_half_height = 0, double right_half_height = 0, double length = 0, double r = 0) 
		: left_half_height_(left_half_height), right_half_height_(right_half_height), length_(length), r_(r)
	{
		shape_type_ = kOval;
		shape_length_ = length_;
...
protected:
	const ShapeDescribe* shape_describe_;
