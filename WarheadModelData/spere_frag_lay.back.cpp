...
void LineShapeLay::LayPerLayer(unsigned layer_index)
{
	...
	//Bit_Point2D endpoint = datum_point_ + Bit_Point2D(0, away_left_h_) + RotateV(per_layer_v_, 90) * (r_+layer_index*2*r_);
	//Bit_Point2D rotate_c = datum_point_ + Bit_Point2D(0, away_left_h_) + per_layer_v_ * (0+r_);
	//修正布设端点，从低到高布设
	...
	for(unsigned i = 0; i<per_layer_num_; ++i)
	{
		...
		Bit_Point2D p2d = RotateV(endpoint, -slope_rad_);
		...
		//	layer_frags.push_back(CircleFrags());
		//	CircleFrags& frags_circle = layer_frags.back();
		//	Bit_Point2D p2d = endpoint + RotateV(per_layer_v_, 30)*2*r_;
		//	frags_circle.push_back(CoordinatesPosition(p2d.Get_x(), p2d.Get_y(), 0));
		//	circle_radius = p2d.Get_y()-r_;
		//	frags_circle_radius_->push_back(circle_radius);
		//	layer_normals.push_back(normal_angle_); 
...
void LineShapeLay::UpdateVar() 
{
	...
	Bit_SpaceCompute2D sc2d;
	normal_angle_ = sc2d.ComputeAngleTwoV(RotateV(per_layer_v_, 90), Bit_Point2D(1, 0));
}