...
ShapeDescribe* ShapeDescribe::CreateShape(const Bit_UnitSystem& us, ShapeType type, const QString& describe)
{
	...
	case kCone:
		{
			...
			shape = new ConeDescribe(us, strs[2].toDouble(), strs[3].toDouble(), strs[4].toDouble());
		}
		break;
	case kCylinder:
		{
			Q_ASSERT(strs.size() == 4);
			shape = new CylinderDescribe(us, strs[2].toDouble(), strs[3].toDouble());
		}
		break;
	case kOval:
		{
			Q_ASSERT(strs.size() == 6);
			shape = new OvalDescribe(us, strs[2].toDouble(), strs[3].toDouble(), strs[5].toDouble(), strs[4].toDouble());
		}
		break;
	}
	if(shape)
...
{
	if(!shape_describe_.valid() || num < 1)
		return ;
	InitEquation();
	points.resize(num);
	double dx = 0;
...
void CurveEquation::InitEquation()
{
	...
	const ConeDescribe* cone = dynamic_cast<const ConeDescribe*>(shape_describe_.get());
	const CylinderDescribe* cylinder = dynamic_cast<const CylinderDescribe*>(shape_describe_.get());
	const OvalDescribe* oval = dynamic_cast<const OvalDescribe*>(shape_describe_.get());
	if(cone)
	{
...
double LineEquation::CountVolume(double left_h, double right_h) const
{
	...
	if(a_ == 0)
	{
		return - (1/3.0) * M_PI *(c-d)*(3*pow(b_, 2)+3*b_*(h1+h2) + pow(h1, 2) + h1*h2 + pow(h2, 2));
	}
